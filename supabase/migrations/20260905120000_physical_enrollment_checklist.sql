...
-- No scan or upload is required at the counter.
...
SET status = CASE WHEN status IN ('submitted', 'validated') THEN 'provided' ELSE 'missing' END;

ALTER TABLE public.enrollment_documents
  ADD CONSTRAINT enrollment_documents_status_check
  CHECK (status IN ('missing', 'provided'));

-- file_url is intentionally kept for a future digital-document phase.
-- The physical form does not populate it.
...
BEGIN
  ...
  FROM public.enrollment_documents WHERE enrollment_id = a.id;

  a.completeness_pct := CASE WHEN req_count = 0 THEN 100 ELSE round(100.0 * provided_count / req_count) END;

  SELECT s.* INTO candidate FROM public.sections s
  WHERE s.level_id = a.requested_level_id AND s.seats_taken < s.capacity
  ORDER BY (s.seats_taken::numeric / nullif(s.capacity,0)) ASC, s.name ASC LIMIT 1;
  ...
  ELSE
    a.recommended_section_id := NULL; a.recommendation_score := 0;
  ...
  SET completeness_pct = a.completeness_pct, duplicate_risk_score = risk, duplicate_flags = flags,
      recommended_section_id = a.recommended_section_id, recommendation_score = a.recommendation_score,
      recommendation_reason = a.recommendation_reason, updated_at = now()
  WHERE id = a.id;
