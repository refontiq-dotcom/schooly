  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-6 space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-xs text-slate-500">{p.step === "send" ? "Connectez-vous avec votre email. Aucun mot de passe nécessaire." : "Entrez le code reçu par email."}</p>
      </div>
      {p.step === "send" && (
      ...
      {p.step === "verify" && (
        <form action={p.verifyAction} className="space-y-4">
          ...
          <button type="button" onClick={() => p.setStep("send")} className="w-full text-xs text-slate-500 hover:text-slate-700">← Changer d&apos;email</button>
        </form>
      )}
    </div>
  )
