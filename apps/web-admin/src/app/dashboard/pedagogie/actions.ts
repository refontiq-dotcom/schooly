...
if (!roleData?.school_id) return { error: "Aucune ��cole rattach��e" }
...
const roleData = await getSchoolId(user.id)
if (!roleData?.school_id) return { error: "Aucune cole rattache" }
...
// On rcupre les utilisateurs qui ont le rle professeur
...
// Rcuprer toutes les notes pour cet lve, cette matire et cette anne
...
// Calcul pondr : somme(note * poids) / somme(poids)
...
return {
data: {
average: Math.round(average * 100) / 100, // arrondir  2 dcimales
...
// Rcuprer tous les enrollments pour cette classe
...
// Pour chaque lve, calculer la moyenne gnrale
const results: any[] = []

for (const enrollment of enrollments) {
// Rcuprer toutes les matires pour cette classe
...
student_name: `${enrollment.students?.[0]?.first_name ?? ""} ${enrollment.students?.[0]?.last_name ?? ""}`,
average: 0,
})
continue
}

// Calculer la moyenne pour chaque matire
