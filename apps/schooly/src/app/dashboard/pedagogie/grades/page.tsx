...
import { getEvaluationConfiguration, createEvaluationRule, createEvaluationPeriod, closeEvaluationPeriod, getPeriodResults, type PeriodResult } from "../evaluation-actions"
...
export default function EvaluationPanel() {
  ...
  const selectedRule = rules.find(r => r.id === periods.find(p => p.id === selectedPeriod)?.rule_id)
  async function reload() {
    const [configuration, yearRes, classRes, subjectRes, enrollmentRes, gradeRes] = await Promise.all([
      getEvaluationConfiguration(), getAcademicYearsForSchool(), getClassesForSchool(), getSubjectsForSchool(), getEnrollmentsForSchool(), getGradeEntries(),
    ])
    const error = configuration.error ?? yearRes.error ?? classRes.error ?? subjectRes.error ?? enrollmentRes.error ?? gradeRes.error
    ...
    setEnrollments(enrollmentRes.data ?? []); setGrades(gradeRes.data ?? []); setReady(true)
  }
  useEffect(() => {
    if (!user) return
    let active = true
  ...
  return <div className="space-y-6">
    ...
    {ready && <section className="space-y-4 rounded border p-4">
      <h2 className="text-xl font-semibold">3. Saisir une note ou une absence justifiée</h2>
      ...
      <ActionForm action={mutate(createGradeEntry)} className="grid gap-4 sm:grid-cols-3">
        <Field label="Période"><Select name="periodId" required value={selectedPeriod} onChange={e => setSelectedPeriod(e.target.value)}><option value="">Choisir</option>{periods.filter(p => !p.locked_at).map(p => <option key={p.id} value={p.id}>{periodLabel(p)}</option>)}</Select></Field>
        <input type="hidden" name="academicYearId" value={selectedRule?.academic_year_id ?? ""} />
        ...
        <Field label="Intitulé unique de l’évaluation"><Input name="label" required /></Field>
        ...
        <Field label="Barème de l’évaluation"><Input name="maxValue" type="number" min="0.01" step="0.01" defaultValue="20" required /></Field>
        ...
        <Button type="submit" disabled={!selectedRule}>Enregistrer</Button>
      </ActionForm>
    </section>}
    {ready && direction && <section className="space-y-4 rounded border p-4">
      <h2 className="text-xl font-semibold">4. Moyennes provisoires par période</h2>
      <p>Aucune admission automatique : les évaluations attendues et la validation annuelle restent à raccorder. Les coefficients sont ceux de la matrice actuelle.</p>
      ...
      {results && <ul>{results.length === 0 && <li>Aucune inscription pour cette classe et cette année.</li>}{results.map(r => <li className="border-t py-2" key={r.enrollmentId}>{r.name} — {r.average === null ? "Non calculable" : `${r.average.toFixed(2)}/${r.scale}`} — matières {r.coverage} — provisoire</li>)}</ul>}
    </section>}
    {ready && <section className="space-y-3 rounded border p-4">
      ...
      {grades.map(g => <div className="border-t py-2" key={g.id}>
