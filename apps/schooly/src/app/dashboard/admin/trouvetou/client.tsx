                ...
                <div className="divide-y divide-border/50">
                  {reservations.map((r: any) => (
                    <div key={r.id} className="flex flex-col gap-3 py-4 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <div className="flex items-center gap-2"><Badge variant="outline">{reservationLabel(r.status)}</Badge><Button size="sm" variant="outline" onClick={() => { setSelectedReservation(r); setModal("reservation") }}><Eye className="mr-1 h-4 w-4" /> Détails</Button>{r.status === "reserved" && <Button size="sm" onClick={async () => { setLoading(true); try { const res = await fetch("/api/v1/admin/trouvetou/reservations/finalize",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reservation_id:r.id})}); if(!res.ok) throw new Error(); toast.success("Réservation finalisée"); router.refresh() } catch { toast.error("Impossible de finaliser") } finally { setLoading(false) } }} disabled={loading}>Finaliser</Button>}</div>
                    </div>
                  ))}
                </div>
              )}
