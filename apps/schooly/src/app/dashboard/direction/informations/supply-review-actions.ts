...
export async function getSupplyProposalsForDirection():Promise<Result<SupplyReviewRow[]>>{
 ...
 const {data,error}=await a.from("school_supply_proposals").select("id,assignment_id,class_id,subject_id,teacher_id,status,revision,review_note,configurations,classes(name),subjects(name),users(full_name)").eq("school_id",ctx.schoolId).eq("academic_year_id",y.id).is("deleted_at",null).order("updated_at",{ascending:false})
