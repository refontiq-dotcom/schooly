...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-2xl border border-border/60 bg-card/80 py-(--card-spacing) text-sm text-card-foreground shadow-[0_10px_30px_oklch(0.2_0.05_252_/_0.06)] backdrop-blur-sm transition-shadow duration-200 hover:shadow-[0_14px_36px_oklch(0.2_0.05_252_/_0.09)] [--card-spacing:--spacing(5)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(4)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-2xl *:[img:last-child]:rounded-b-2xl",
        className
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-2xl border-t border-border/60 bg-muted/35 p-(--card-spacing)",
        className
      )}
      {...props}
    />
