...
const RES_ID = "22222222-0000-4000-8000-000000000001";
...
describe("POST /api/reservations/:id/confirm — idempotence", () => {
  ...
  it("retourne 404 si la réservation n'existe pas", async () => {
    ...
    const { request, params } = buildRequest(RES_ID);
  ...
  it("retourne 409 si la réservation est annulée", async () => {
    mockMaybeSingle.mockResolvedValueOnce({
      data: { id: RES_ID, status: "cancelled" },
      error: null,
    });
  ...
  it("est idempotent : pas de second appel à reserve_seat si déjà reserved", async () => {
    mockMaybeSingle.mockResolvedValueOnce({
      data: { id: RES_ID, status: "reserved" },
      error: null,
    });
    mockSingle.mockResolvedValueOnce({
      data: { id: RES_ID, status: "reserved", payment_reference: "PREV-1" },
      error: null,
    });

    const { request, params } = buildRequest(RES_ID, { payment_reference: "PREV-1" });
    const res = await POST(request, { params });
    const json = await res.json();
  ...
  it("appelle reserve_seat quand le statut est pending_payment", async () => {
    mockMaybeSingle.mockResolvedValueOnce({
      data: { id: RES_ID, status: "pending_payment" },
      error: null,
    });
    ...
    const { request, params } = buildRequest(RES_ID, { payment_reference: "PAY-1", amount_paid: 5000 });
    const res = await POST(request, { params });
    const json = await res.json();

    expect(res.status).toBe(200);
    expect(json.already_reserved).toBe(false);
    expect(mockRpc).toHaveBeenCalledWith("reserve_seat", { p_reservation_id: RES_ID });
  });
