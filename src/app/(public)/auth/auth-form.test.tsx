...
async function navigateToEstablishmentLogin() {
  ...
  await user.click(screen.getByRole("button", { name: /Établissement scolaire/ }));
  return user;
}
...
describe("AuthForm – rôle selecteur et formulaire", () => {
  ...
  describe("flux parent – connexion par téléphone + code PIN", () => {
    it("affiche le formulaire tél + code après avoir cliqué 'Je suis parent'", async () => {
      const user = userEvent.setup();
      render(<AuthForm />);
    ...
    it("appelle signInParent avec le téléphone + le code PIN", async () => {
      ...
      expect(formData.get("code")).toBe("1234");
    });

    it("affiche le message d'erreur quand signInParent échoue", async () => {
      mockSignInParent.mockResolvedValue("Code d'accès invalide. Réessayez.");
      ...
      await waitFor(() => {
        expect(screen.getByText(/Code d'accès invalide/i)).toBeInTheDocument();
      });
