...
create index if not exists idx_preinscription_documents_reservation
  on public.preinscription_documents (reservation_id);
create index if not exists idx_preinscription_documents_school_status
  on public.preinscription_documents (school_id, status);

drop trigger if exists trg_preinscription_documents_updated_at on public.preinscription_documents;
create trigger trg_preinscription_documents_updated_at
  before update on public.preinscription_documents
...
create policy preinscription_documents_member_read on public.preinscription_documents
  for select using (is_super_admin() or is_school_member(school_id));

drop policy if exists preinscription_documents_direction_write on public.preinscription_documents;
create policy preinscription_documents_direction_write on public.preinscription_documents
  for all
...
as $$
begin
  insert into public.preinscription_documents (
    school_id, reservation_id, required_document_id, document_label, required
  )
  select d.school_id, new.id, d.id, d.nom, d.obligatoire
  from public.required_documents d
  where d.school_id = new.school_id
...
create trigger trg_snapshot_required_documents
  after insert on public.trouvetou_reservations
  for each row execute function public.snapshot_required_documents_for_reservation();

-- Utilisé uniquement côté serveur pour rattraper une préinscription créée
-- avant cette migration. Pas de fichier, uniquement des métadonnées administratives.
...
begin
  ...
  join public.required_documents d
    on d.school_id = r.school_id
   and d.deleted_at is null
